...
if medicatie is not None:
    ...
    col_middel = None
    for zoek in ["middel", "geneesmiddel", "product", "medicatie"]:
        if zoek in cols_lower:
            col_middel = medicatie.columns[cols_lower.index(zoek)]
            break
    if col_middel:
        top_middelen = (
            medicatie.groupby(col_middel).size()
            .reset_index(name="Aantal")
            .sort_values("Aantal", ascending=False)
            .head(10)
        )
        fig = px.bar(top_middelen, x=col_middel, y="Aantal")
        st.plotly_chart(fig, use_container_width=True)
    else:
...
if leeftijden is not None:
    ...
    if "leeftijd" in cols_lower:
        ...
        fig = px.histogram(leeftijden, x=colnaam, nbins=10, title="Verdeling van leeftijden")
        st.plotly_chart(fig, use_container_width=True)
    else:
